 ...
 * 참고: https://minhamina.tistory.com/65
 */
public class Problem17143 {
	...
	private static int dx[] = {-1, 0, 1, 0}; //상 좌 하 우 순
	...
	public static void main(String[] args) throws IOException {
		...
		for (int col = 0; col < C; col++) { // 열의 끝까지 반복
			...
			for (int row = 0; row < R; row++) {
				if (map[row][col] != null) {
					answer += map[row][col].size; // 2. 가장 가까운 상어 크기 정답 변수에 저장
			...
			while (!queue.isEmpty()) {
				...
				// 속력만큼 상어 이동 시키기
				int speed = shark.speed; // 시간초과로 최소한의 이동을 위해 나머지 연산
				if (shark.direction == 0 || shark.direction == 2) //상 하
					speed %= (R - 1) * 2;
				else if (shark.direction == 1 || shark.direction == 3) //좌 우
					speed %= (C - 1) * 2;

				for (int s = 0; s < speed; s++) {
					// 현재 r, c에 방향에 맞게 1칸씩 추가하며 위치 이동
					int nr = shark.row + dx[shark.direction];
					int nc = shark.col + dy[shark.direction];

					// 이동할 새로운 위치가 범위를 벗어나 벽에 부딪히면
					if (nr < 0 || nr >= R || nc < 0 || nc >= C) {
						shark.row -= dx[shark.direction]; // 다시 값 돌려주고
						shark.col -= dy[shark.direction];
				...
				if (map[shark.row][shark.col] != null) { // 이미 상어가 있다면 두 상어 크기 비교
					if (map[shark.row][shark.col].size < shark.size) { // 기존 상어보다 현재 상어가 크다면
						map[shark.row][shark.col] = new Shark(shark.row, shark.col, shark.speed, shark.direction, shark.size); // 현재 상어 넣어줌
					}
